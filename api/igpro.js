export default async function handler(req, res) {
    ...
    // Is baar hum 'url' parameter le rahe hain query string se
    const { url } = req.query;

    if (!url) {
        return res.status(400).json({ 
            error: "Please provide an Instagram Video Post And username or URL.", 
            usage: "/api/?url=INSTAGRAM_LINK_HERE" 
        });
    }
